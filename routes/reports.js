...
// routes/reports.js içindeki listeleme endpoint'i
router.get('/api/reports/list', (req, res) => {
    const { project } = req.query;
    const selectedProj = project || 'Auth Service';
    ...
    db.all(selectQuery, [], (err, rows) => {
        ...
        // 🚀 KORUMA: Hem projeye göre filtrele hem de diskte fiziksel dosyası var mı diye bak!
        const filteredRows = rows.filter(row => {
            const filePath = path.join(reportFolder, row.log_file_name);
            const fileExists = fs.existsSync(filePath); // Klasörde var mı?

            if (!fileExists) return false; // Dosya fiziksel olarak yoksa listeden uçur

            if (row.log_file_name.includes('___')) {
                const [projName] = row.log_file_name.split('___');
                return projName === selectedProj;
            }
            return selectedProj === 'Auth Service';
        });
