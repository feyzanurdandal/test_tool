...
// UI Testleri için Hata Tespiti
const isUiTestFailed = (logContent) => {
    if (!logContent) return false;
    const lowerLog = logContent.toLowerCase();
    
    const errorKeywords = [
        'error:', 'failed', 'exception:', 'cannot find',
        'incorrect api key', 'failed to launch', 'timeout',
        'element not found', 'execution context was destroyed',
        'hata oluştu', 'başarısız', 'uyarı:', 'geçersiz',
        'alert:', 'modal-error', 'lütfen kontrol edin'
    ];
...
// ─── 3. API: PROJE BAZLI SENARYOLARI LİSTELEME (FAZ 2: testType filtresi eklendi) ───
router.get('/list', requireAuth, requireProjectAccess, async (req, res, next) => {
    const project = req.query.project || req.query.projectName;
    ...
    try {
        ...
        if (scenariosRes.success && scenariosRes.data) {
            // FAZ 2: test_tipi filtresi (varsayılanı UI olan eski kayıtlar da gözetilir)
            const filteredScenarios = scenariosRes.data
                .filter(s => {
                    const rowType = (s.test_tipi || 'UI').toUpperCase();
                    return rowType === testType;
                })
                .map(s => s.senaryo_adi);
...
// ─── 4. API: SENARYO İÇERİĞİNİ OKUMA (FAZ 1: contentTr Türkçe adımlar eklendi) ───
router.get('/content', requireAuth, validate(getScenarioContentSchema), requireProjectAccess, async (req, res, next) => {
    const scenarioName = req.query.scenarioName;
    const selectedProj = (req.query.project || req.query.projectName || 'Varsayılan Proje').trim();
    ...
    try {
        ...
        if (scenarioRes.success && scenarioRes.data && scenarioRes.data.length > 0) {
            ...
            return res.json({ 
                ...
                contentTr: scenario.adimlar_tr || null, // 💡 FAZ 1: Ham Türkçe metin
...
// ─── 5. API: SENARYO KAYDETME VE AI ÇEVİRİSİ (FAZ 1 & 2: İki Aşamalı Kayıt) ───
// ─── 5. API: SENARYO KAYDETME VE AI ÇEVİRİSİ (Hatasız ve Sağlamlaştırılmış) ───
router.post('/create-and-save', aiCallLimiter, requireAuth, validate(createScenarioSchema), requireProjectAccess, async (req, res, next) => {
    ...
    try {
        ...
        // Proje altında aynı isimde senaryo var mı kontrolü
        const checkScenario = await dpu.selectWhere('senaryolar', {
            project_id: { eq: projectId },
            senaryo_adi: { eq: scenarioName }
        });
        ...
        // 💡 1. AŞAMA: Ham Türkçe metinle kaydı oluştur (null yerine boş string "" kullanıldı)
        const insertPayload = {
            project_id: projectId,
            senaryo_adi: scenarioName,
            hedef_url: targetUrl,
            adimlar_tr: rawTrText,
            adimlar: "", // null gönderilmez, boş string veritabanı kısıtını aşar
        ...
        console.log("DPU Base Senaryo Kayıt İsteği:", insertPayload);
        const insertResult = await dpu.insert('senaryolar', insertPayload);
        
        if (!insertResult.success) {
            console.error("DPU Base Insert Hatası Detayı:", insertResult);
            return res.status(500).json({ 
                error: "Senaryo veritabanına eklenemedi.", 
                details: insertResult.error || insertResult.message || "Bilinmeyen DB hatası" 
            });
        }

        // Yeni oluşturulan kaydın ID'sini yakala
        ...
        console.error("Senaryo oluşturma genel sunucu hatası:", error);
        next(error);
    }
});

// ─── 5.1 API: SENARYO GÜNCELLEME (FAZ 1 & 2: İki Aşamalı Güncelleme) ───
router.post('/update', aiCallLimiter, requireAuth, validate(updateScenarioSchema), requireProjectAccess, async (req, res, next) => {
    ...
    try {
        ...
        // 💡 FAZ 1 - AŞAMA 1: Türkçe adımları ve metadata'yı güncelle
        const updatePayload = {
            senaryo_adi: scenarioName,
            hedef_url: targetUrl,
            adimlar_tr: rawTrText,
        ...
        // 💡 FAZ 1 - AŞAMA 2: Çeviriyi yeniden yap ve JSON'ı mühürle
        const stagehandJson = await translateToStagehandJson(turkishInstructions, targetUrl);
        if (stagehandJson) {
...
// ─── 7. API: TEKİL TESTİ PLAYWRIGHT İLE KOŞTURMA (FAZ 2 & 3: test_tipi raporlaması) ───
router.post('/run', testRunLimiter, requireAuth, validate(runScenarioSchema), requireProjectAccess, async (req, res, next) => {
    const { scenarioName, targetUrl, projectName } = req.body;
    ...
    try {
        ...
        const hasErrorInLog = isUiTestFailed(safeLogContent);
        const finalStatus = (testResult.isSuccess && !hasErrorInLog) ? "SUCCESS" : "FAILED";

        const reportData = {
            project_id: projectId,
            scenario_name: scenarioName,
            test_tipi: scenarioTestType, // 💡 FAZ 2 & 3: Raporun test tipi
            status: finalStatus,
            log_content: safeLogContent,
            created_at: new Date().toISOString()
        };
...
// ─── 8. API: PROJE BAZLI RAPORLARI LİSTELEME (FAZ 2: testType filtresi) ───
router.get('/reports/list', requireAuth, requireProjectAccess, async (req, res, next) => {
    const project = req.query.project || req.query.projectName;
    const testType = (req.query.testType || 'UI').toUpperCase();
    const selectedProj = (project || '').trim();
...
// ─── 12. API: TEKİL TEST RAPORUNU SİLME (FAZ 0: Temizlenmiş yetki denetimi) ───
router.post('/reports/delete', requireAuth, async (req, res, next) => {
    const { id } = req.body;
    if (!id) return res.status(400).json({ error: "Eksik parametre! Rapor ID değeri gelmedi." });
    ...
    try {
        ...
        // Admin değilse raporun ait olduğu projeye kullanıcının erişim izni var mı denetle
        if (req.user.role !== 'ADMIN') {
            const projectRes = await dpu.selectWhere('projeler', { id: { eq: report.project_id } });
            if (projectRes.success && projectRes.data && projectRes.data.length > 0) {
                const projectName = projectRes.data[0].proje_adi;
...
// ─── 12.1 API: TOPLU TEST RAPORLARINI SİLME (FAZ 0: Optimize edilmiş yetki denetimi) ───
router.post('/reports/delete-batch', requireAuth, async (req, res, next) => {
    const { ids } = req.body;
