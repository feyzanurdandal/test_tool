...
//                         // AI geçiştirme yaptıysa kontrol et
//                         if (resultStr.includes('nextChunk') || resultStr.includes('prevChunk') || resultStr.includes('"action":null')) {
//                             throw new Error(`ACTION_FAILED: Yapay zeka "${step.instruction}" adımında hedef elemanı bulamadı.`);
//                         }
//                         actSuccess = true;
//                     } catch (actError: any) {
//                         const errText = actError.message || actError.toString();

//                         // 💡 3. ZOD ŞEMA / REGEX HATASI KURTARMA (invalid_format / twoStep undefined vb.)
...
//                             errText.includes('twoStep');

//                         if (isZodSchemaError) {
//                             console.warn(`⚠️ Model Zod formatına takıldı, akıllı fallback devrede: ${step.instruction}`);

//                             const matchText = step.instruction.match(/"([^"]+)"/);
//                             if (matchText && matchText[1]) {
//                                 const targetText = matchText[1];
//                                 try {
//                                     // 💡 1. Büyük/küçük harf duyarsız (Case-insensitive) olarak metne tıkla
//                                     const textLocator = pwPage.getByText(new RegExp(`^${targetText}$`, 'i')).first();
//                                     await textLocator.click({ force: true, timeout: 2500 });
//                                     console.log(`✅ Fallback ile "${targetText}" tıklandı.`);
//                                     actSuccess = true;
//                                 } catch {
//                                     try {
//                                         // 💡 2. Eğer metin bir label/başlık ise (Örn: "Roller"), ilgili PrimeNG dropdown kutusuna tıkla
//                                         const dropdownLocator = pwPage.locator(`p-dropdown:has-text("${targetText}"), p-multiselect:has-text("${targetText}"), [placeholder*="${targetText}" i]`).first();
//                                         await dropdownLocator.click({ force: true, timeout: 2500 });
//                                         console.log(`✅ Fallback ile "${targetText}" dropdown kutusuna tıklandı.`);
//                                         actSuccess = true;
//                                     } catch (clickErr) {
//                                         console.warn(`Fallback tıklayamadı:`, (clickErr as Error).message);
//                                         throw actError;
//                                     }
//                                 }
//                             } else {
...
//                     // 💡 4. Dropdown veya açılır menü animasyonu beklemesi
//                     const isDropdownClick = step.instruction.toLowerCase().includes('dropdown') || 
//                                             step.instruction.toLowerCase().includes('menu') ||
//                                             step.instruction.toLowerCase().includes('menü');
                    
//                     if (isDropdownClick) {
//                         console.log("⏳ Açılır menü animasyonu bekleniyor (800ms)...");
//                         await pwPage.waitForTimeout(800);
//                     }

//                     // 💡 5. Fakülte seçiminden sonra alt programların API'den yüklenmesini bekle
//                     const isFacultySelection = step.instruction.toLowerCase().includes('fakülte') || 
//                                                step.instruction.toLowerCase().includes('faculty');
//                     if (isFacultySelection) {
//                         console.log("⏳ Fakülteye bağlı program listesi yükleniyor (1.5sn)...");
...
//                                               step.instruction.toLowerCase().includes('select');
//                     if (isNavigationStep) {
//                         await pwPage.waitForTimeout(1500); 
//                     }
