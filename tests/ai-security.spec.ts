...
test('Yapay Zeka Test Otomasyonu', async () => {
    ...
    try {
        ...
        // dpu nesnesinin asıl tipini ve varlığını emniyete alıyoruz kanka
        const dpuClient = dpu as any;
        
        // Önce aktif test sağlayıcısını seçelim
        const activeRunnerRes = await dpuClient.select('ayarlar', 1, 'ayar_anahtar:eq:test_runner_api');

        if (activeRunnerRes.success && activeRunnerRes.data.length > 0) {
            chosenApi = activeRunnerRes.data[0].ayar_deger;
            console.log(`🎯 [Test Runner] Aktif Çalıştırıcı Sağlayıcı: ${chosenApi}. Key ve Model tek satırdan çekiliyor...`);

            // Sağlayıcıya ait satırı nokta atışı tek sorgu ile çekip hem key hem de model bilgisini alıyoruz
            const providerRes = await dpuClient.select('ayarlar', 1, `ayar_anahtar:eq:${chosenApi}`);
    ...
    const stagehand = new Stagehand({
        ...
        domSettleTimeout: 10000,
        localBrowserLaunchOptions: { headless: false },
        // DPU Qwen seçildiyse customBaseUrl endpoint geçişini yapıyoruz kanka
        ...(customBaseUrl ? { 
            configuration: {
                baseURL: customBaseUrl
            }
        ...
        for (const step of promptData.steps) {
            try {
                if (step.type === 'act') {
                    ...
                    // 🌟 GÜVENLİK ÖNLEMİ: Boş veya tanımsız step.field parametresinden kaynaklanan 'undefined' pürüzünü eziyoruz! 🔒
