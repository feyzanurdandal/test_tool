...
// 💡 Pop-up ve Modal Hata Kelimeleri Listesi
...
test('Yapay Zeka Test Otomasyonu', async () => {
    ...
    pwPage.on('dialog', async (dialog) => {
        ...
        console.log(`🔔 Sayfada Pop-up (Dialog) Belirdi: "${dialogText}"`);
        await dialog.dismiss();
        
        const lowerDialog = dialogText.toLowerCase();
        if (ERROR_KEYWORDS.some(kw => lowerDialog.includes(kw))) {
            throw new Error(`CRITICAL_POPUP_ERROR: Ekranda hata uyarı dialogu çıktı: "${dialogText}"`);
        }
    });
        ...
        for (const step of promptData.steps) {
            try {
                if (step.type === 'act') {
                    ...
                    const response = await stagehand.extract(step.instruction, dynamicSchema, { page: pwPage });
                    const extractedValue = (response && response[fieldName]) ? String(response[fieldName]) : "";
                    ...
                    // 💡 KRİTİK KONTROL: Çekilen metin bir hata veya uyarı pop-up mesajı mı?
                    const lowerExtracted = extractedValue.toLowerCase();
                    const detectedKeyword = ERROR_KEYWORDS.find(kw => lowerExtracted.includes(kw));

                    if (detectedKeyword) {
                        console.error(`❌ [HATA TESPİT EDİLDİ] Ekrandan çekilen veri bir HATA/UYARI mesajı içeriyor! ("${detectedKeyword}" kelimesi yakalandı)`);
                        throw new Error(`CRITICAL_POPUP_ERROR: Ekrandaki pop-up/mesaj bir hataya işaret ediyor: "${extractedValue}"`);
                    }
                }
            } catch (e: any) {
                // Eğer hata bizim fırlattığımız pop-up hatasıysa tekrar denemeden testi patlat
                if (e.message && e.message.includes('CRITICAL_POPUP_ERROR')) {
                    throw e;
                }
