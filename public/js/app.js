...
//     // ─── GLOBAL PROJE DÜZENLEME FONKSİYONU ───
// window.editProject = async function(oldProjectName) {
//     if (!oldProjectName || oldProjectName === 'Varsayılan Proje') {
//         alert("Varsayılan projenin adı değiştirilemez!");
//         return;
//     }

//     const newProjectName = prompt(`"${oldProjectName}" projesi için yeni adı girin:`, oldProjectName);
//     if (!newProjectName || newProjectName.trim() === "" || newProjectName.trim() === oldProjectName) return;

//     try {
//         const userSession = JSON.parse(localStorage.getItem("test_user") || "{}");
//         const response = await fetch('/api/scenarios/projects/update', {
//             method: 'POST',
//             headers: {
//                 'Content-Type': 'application/json',
//                 'X-User-Token': userSession.token || ""
...
//             await loadProjects();
//         } else {
//             alert(`Hata: ${result.error || 'Proje adı güncellenemedi.'}`);
//         }
//     } catch (err) {
//         console.error("Proje düzenleme hatası:", err);
//         alert("Sunucuyla iletişim kurulurken bir hata oluştu.");
//     }
// };
...
//                             // Mevcut adımlar doğrudan input kutularına dolduruluyor - kopyala/düzenle mantığı.
//                             // Not: bunlar AI'ın çevirdiği İngilizce metin (orijinal Türkçe DB'de tutulmuyor),
//                             // istersen İngilizce bırakıp gönderebilirsin ya da Türkçeye çevirip düzenleyebilirsin.
//                             if (stepsContainer) {
//                                 stepsContainer.innerHTML = "";
//                                 const steps = (adimlar.steps && adimlar.steps.length > 0) ? adimlar.steps : [{ instruction: "" }];
//                                 steps.forEach(s => {
...
//             const modalTitle = document.getElementById("scenario-modal-title");
//             if (modalTitle) modalTitle.innerHTML = `<i data-lucide="sparkles" class="w-4 h-4 text-[#3b82f6]"></i> Yeni Senaryo Oluştur (<span class="current-project-label"></span>)`;
//             const submitLabel = document.getElementById("save-scenario-submit-btn-label");
...
//             <input type="text" required placeholder='Talimatı girin (Örn: "Arama" kutusunu bul).' value="${escapeHtml(value)}" class="step-input flex-1 bg-transparent text-xs text-white outline-none">
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Durum Değişkenleri
    let currentProject = "";
    let currentTestType = "UI"; // 💡 FAZ 2: Aktif test tipi ('UI' | 'SECURITY')
    let batchQueue = [];
    let cachedAllProjects = [];
    let globalEditUserId = "";
    let globalEditScenarioName = "";
    ...
    // ─── 1. SENARYOLARI ÇEKİP TÜRKÇE ADIMLARI VE KARTLARI BASAN FONKSİYON (FAZ 1 & 2) ───
    async function loadScenarios() {
        if (!currentProject || currentProject === "Varsayılan Proje") {
            return;
        }
        ...
        try {
            ...
            // 💡 FAZ 2: testType filtresi eklendi
            ...
            if (result.scenarios && result.scenarios.length > 0) {
                ...
                result.scenarios.forEach((scenarioName, index) => {
                    ...
                    // 💡 FAZ 1: Türkçe adımları akordeonda satır satır gösterme
                    row.addEventListener("click", async () => {
                        const contentEl = document.getElementById(contentId);
                        const chevronIcon = row.querySelector(".chevron-icon-scen");
                        const isOpen = !contentEl.classList.contains("hidden");
                            ...
                            if (detailsContainer.getAttribute("data-loaded") !== "true") {
                                try {
                                    ...
                                    if (contentResult.success) {
                                        ...
                                        // 💡 FAZ 1: Eğer Türkçe ham metin varsa onu bas, yoksa eski format uyarısı ver
                ...
                // A0. DÜZENLEME BUTONLARI (FAZ 1: Türkçe adımları textarea/inputlara doldurma)
                document.querySelectorAll(".edit-scenario-btn").forEach(btn => {
                    btn.addEventListener("click", async (e) => {
                        e.stopPropagation();
                        const scenarioName = btn.getAttribute("data-name");
    ...
    // Başlık ve Rozetleri Güncelleyen Yardımcı Fonksiyon (FAZ 2)
    ...
    // ─── NAVİGASYON TIKLAMA OLAYLARI (FAZ 2: UI & SECURITY AYRIMI) ───
    navButtons.forEach(btn => {
        btn.addEventListener("click", () => {
            const userSession = JSON.parse(localStorage.getItem("test_user") || "{}");
            const targetViewId = btn.getAttribute("data-target");
    ...
    if (openNewScenarioBtn) {
        openNewScenarioBtn.addEventListener("click", () => {
            ...
            // 💡 FAZ 3: Siber güvenlik beklenen sonuç seçim kutusu görünürlüğü
    ...
    // Mevcut Senaryodan Adımları Çekip İçe Aktarma Buton Olayı (FAZ 1: Türkçe adımları aktarır)
    const importStepsBtn = document.getElementById("import-scenario-steps-btn");
    if (importStepsBtn) {
        importStepsBtn.addEventListener("click", async () => {
            const importDropdown = document.getElementById("import-scenario-dropdown");
    ...
    // ─── SENARYO KAYDETME / GÜNCELLEME FORMU (FAZ 1, 2, 3 Uyumlu) ───
    if (scenarioForm) {
        scenarioForm.addEventListener("submit", async (e) => {
            e.preventDefault();
            ...
            const bodyData = {
                ...
                targetUrl,
                projectName: activeProjectName,
                testType: currentTestType, // 💡 FAZ 2: 'UI' veya 'SECURITY'
                expectedOutcome: expectedOutcomeSelect ? expectedOutcomeSelect.value : "SUCCESS_EXPECTED" // 💡 FAZ 3
            };
            if (isEditMode) bodyData.originalScenarioName = globalEditScenarioName;
